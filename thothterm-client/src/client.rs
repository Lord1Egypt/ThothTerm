...
#[error(
    "Please install the same version of thothterm on both the client and server!\n\
...
impl Reconnectable {
    ...
    /// Resolve the path to thothterm for the remote system.
    ...
    /// `path` comes from the SshDoman::remote_thothterm_path option; if set
    ...
    /// message to the user.
    fn thothterm_bin_path(path: &Option<String>) -> String {
        path.as_deref().unwrap_or("thothterm").to_string()
    }
    ...
    fn ssh_connect(
        ...
        let proxy_bin = Self::thothterm_bin_path(&ssh_dom.remote_thothterm_path);
    ...
    pub fn tls_connect(
        ...
        if let Some(Ok(ssh_params)) = tls_client.ssh_parameters() {
            if self.tls_creds.is_none() {
                ...
                let mut ssh_config = thothterm_ssh::Config::new();
                ...
                let creds = ui.run_and_log_error(|| {
                    ...
                    let cmd = format!(
                        "{} cli tlscreds",
                        Self::thothterm_bin_path(&tls_client.remote_thothterm_path)
                    );
...
impl Client {
        ...
        thread::spawn(move || {
            ...
            loop {
                if let Err(e) = client_thread(&mut reconnectable, local_domain_id, &mut receiver) {
                    ...
                    ui.title("thothterm: Reconnecting...");
            ...
            .await
        {
            ...
            Err(err) => {
                ...
                let msg = if err.root_cause().is::<Timeout>() {
                    ...
                    format!(
                        "Please install the same version of thothterm on both \
    ...
    fn compute_unix_domain(
        ...
        match std::env::var_os("THOTHTERM_UNIX_SOCKET") {
            ...
            Some(_) | None => {
                ...
                Ok(config
                    ...
                    .ok_or_else(|| {
                        anyhow!(
                            "no default unix domain is configured and THOTHTERM_UNIX_SOCKET \
    ...
    pub async fn resolve_pane_id(&self, pane_id: Option<PaneId>) -> anyhow::Result<PaneId> {
        let pane_id: PaneId = match pane_id {
            Some(p) => p,
            None => {
                if let Ok(pane) = std::env::var("THOTHTERM_PANE") {
                    pane.parse()?
                } else {
                    ...
                    if clients.is_empty() {
                        anyhow::bail!(
                            "--pane-id was not specified and $THOTHTERM_PANE
