...
use url::Url;
use thothterm_term::terminal::Alert;
use thothterm_term::StableRowIndex;
        ...
        match decoded.pdu {
            ...
            Pdu::SetClientId(SetClientId {
                ...
                if is_proxy {
                    ...
                    // on from the `thothterm cli list-clients` information
                ...
                match std::env::current_exe().context("resolving current_exe") {
                    ...
                    Ok(executable_path) => {
                        send_response(Ok(Pdu::GetCodecVersionResponse(GetCodecVersionResponse {
                            ...
                            version_string: config::thothterm_version().to_owned(),
                            executable_path,
                            config_file_path: std::env::var_os("THOTHTERM_CONFIG_FILE")
                                .map(Into::into),
                        })))
                    }
                }
