...
use std::rc::Rc;
use thothterm_dynamic::{FromDynamic, ToDynamic, Value as DynValue};
...
pub struct ValueLua {
    pub value: thothterm_dynamic::Value,
}
                ...
                if let Ok(mt) = ud.get_metatable() {
                    if let Ok(to_dynamic) = mt.get::<mlua::Function>("__thothterm_to_dynamic") {
                        return match to_dynamic.call(LuaValue::UserData(ud.clone())) {
                            ...
                            .fmt(fmt),
                            Err(err) => write!(fmt, "Error calling __thothterm_to_dynamic: {err}"),
                        };
                    }
                }
