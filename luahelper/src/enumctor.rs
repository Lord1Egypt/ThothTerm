...
/// The primary usage is to enable `thothterm.action` to have the following
/// behaviors for KeyAssignment:
///
/// `thothterm.action{QuickSelectArgs={}}` -> compatibility with prior versions;
/// the table is passed through and from_dynamic -> lua conversion is attempted.
///
/// `thothterm.action.QuickSelectArgs` -> since the `QuickSelectArgs` variant
/// has a payload that impl Default, this is equivalent to the call above.
///
/// `thothterm.action.QuickSelectArgs{}` -> equivalent to the call above, but
/// explicitly calls the constructor with no parameters.
///
/// `thothterm.action.QuickSelectArgs{alphabet="abc"}` -> configures the alphabet.
///
