    ...
    fn log(&self, record: &Record) {
        if self.filter.matches(record) {
...

            {
                ...
                // <https://github.com/thothterm/thothterm/issues/1839>
...
fn setup_pretty() -> (LevelFilter, Logger) {
    let base_name = std::env::current_exe()
        ...
        .unwrap_or_else(|| "thothterm".to_string());

    if base_name.contains("gui") {
        // Only tidy up logs when the gui process is starting.
        // rationale: `thothterm cli` commands should have as low startup
    ...
    if let Ok(s) = std::env::var("THOTHTERM_LOG") {
        filters.parse(&s);
    } else {
