...
pub fn set_thothterm_executable() {
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            std::env::set_var("THOTHTERM_EXECUTABLE_DIR", dir);
        }
        std::env::set_var("THOTHTERM_EXECUTABLE", exe);
    }
}
...
pub fn fixup_appimage() {
    if let Some(appimage) = std::env::var_os("APPIMAGE") {
        ...
        // `ThothTerm.AppImage foo`, which is super confusing for everyone!
        ...
        // THOTHTERM_EXECUTABLE_DIR is set by `set_thothterm_executable`
        // which is called before `fixup_appimage`
        if let Some(dir) = std::env::var_os("THOTHTERM_EXECUTABLE_DIR") {
        ...
        /// the correct environment when it is launched via thothterm.
        /// However, if we are using the system thothterm to spawn a portable
        /// AppImage then we want these to not take effect.
        fn clean_thothterm_config_env() {
...
pub fn bootstrap() {
    config::assign_version_info(
        thothterm_version::thothterm_version(),
        thothterm_version::thothterm_target_triple(),
    );
    setup_logger();
    register_panic_hook();

    set_thothterm_executable();
    ...
    // thothterm never sets WINDOWID and we don't want to inherit it from a
    // parent process.
    std::env::remove_var("WINDOWID");
    // Avoid vte shell integration kicking in if someone started
    // thothterm or the mux server from inside gnome terminal.
    // <https://github.com/thothterm/thothterm/issues/2237>
