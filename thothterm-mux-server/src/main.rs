...
use thothterm_mux_server_impl::update_mux_domains_for_server;

mod daemonize;

#[derive(Debug, Parser)]
#[command(
    about = "Wez's Terminal Emulator\nhttp://github.com/thothterm/thothterm",
...
pub fn spawn_listener() -> anyhow::Result<()> {
    ...
    for unix_dom in &config.unix_domains {
        std::env::set_var("THOTHTERM_UNIX_SOCKET", unix_dom.socket_path());
        let mut listener = thothterm_mux_server_impl::local::LocalListener::with_domain(unix_dom)?;
        thread::spawn(move || {
            listener.run();
        });
    }
