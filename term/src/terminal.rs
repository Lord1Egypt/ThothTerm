...
use std::sync::Arc;
use thothterm_escape_parser::parser::Parser;
...
pub trait DeviceControlHandler: Send + Sync {
    fn handle_device_control(&mut self, _control: thothterm_escape_parser::DeviceControlMode);
}
...
pub enum Alert {
    ...
    Progress(Progress),
    /// A shell command exited with a non-zero status (via OSC 133;D;N)
    CommandFailed {
        exit_code: i32,
    },
}
...
impl Terminal {
    ...
    pub fn perform_actions(&mut self, actions: Vec<thothterm_escape_parser::Action>) {
        self.state.increment_seqno();
        {
            let mut performer = Performer::new(&mut self.state);
