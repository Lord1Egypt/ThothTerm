...
use thothterm_input_types::{is_ascii_control, IntegratedTitleButtonStyle, KeyboardLedStatus};
...
impl WindowInner {
    ...
    fn update_window_shadow(&mut self) {
        ...
        unsafe {
            ...
            // https://github.com/thothterm/thothterm/issues/310.
            // But allow overriding the shadows independent of opacity as well:
            // <https://github.com/thothterm/thothterm/issues/2669>
            let shadow = if self
                .config
                .window_decorations
...
const VIEW_CLS_NAME: &str = "ThothTermWindowView";
const WINDOW_CLS_NAME: &str = "ThothTermWindow";
...
impl WindowView {
    ...
    extern "C" fn thothterm_perform_key_assignment(
        this: &mut Object,
        _sel: Sel,
        menu_item: *mut Object,
    ) {
        let menu_item = MenuItem::with_menu_item(menu_item);
        // Safe because thothtermPerformKeyAssignment: is only used with KeyAssignment
        let action = menu_item.get_represented_item();
        log::debug!("thothterm_perform_key_assignment {action:?}",);
        match action {
    ...
    fn key_common(this: &mut Object, nsevent: id, key_is_down: bool) {
        ...
        let (prefer_vkey, unmod) =
            if virtual_key == kVK_Delete && modifiers.contains(Modifiers::ALT) {
                ...
                // https://github.com/thothterm/thothterm/issues/739
        ...
        // <https://github.com/thothterm/thothterm/issues/1902>
        ...
        if let Some(key) = key_string_to_key_code(chars).or_else(|| key_string_to_key_code(unmod)) {
                ...
                || (only_right_alt && !send_composed_key_when_right_alt_is_pressed)
            {
                ...
                match (&key, &raw) {
                    ...
                    // <https://github.com/thothterm/thothterm/issues/1891>
    ...
    extern "C" fn perform_key_equivalent(this: &mut Object, _sel: Sel, nsevent: id) -> BOOL {
            ...
            || (chars == "\x19"/* Shift-Tab: See issue #1902 */)
        {
            ...
            // <https://github.com/thothterm/thothterm/issues/1867>
    ...
    extern "C" fn did_resize(this: &mut Object, _sel: Sel, _notification: id) {
        ...
        if let Some(this) = Self::get_this(this) {
            ...
            // thothterm-gui/src/termwindow/resize.rs.
            // <https://github.com/thothterm/thothterm/issues/3503>
    ...
    fn define_class() -> &'static Class {
        ...
        unsafe {
            ...
            cls.add_method(
                sel!(thothtermPerformKeyAssignment:),
                Self::thothterm_perform_key_assignment
