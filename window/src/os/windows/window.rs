...
use std::rc::Rc;
use std::sync::Mutex;
use thothterm_color_types::LinearRgba;
use thothterm_font::FontConfiguration;
use thothterm_input_types::KeyboardLedStatus;
...
impl Window {
    fn create_window(
        ...
        let class = WNDCLASSW {
            ...
            cbWndExtra: 0,
            hInstance: h_inst,
            // FIXME: this resource is specific to the thothterm build and this should
...
unsafe fn update_title_font(hwnd: HWND) {
    ...
    if let Some(lf) = get_title_log_font(hwnd, hdc) {
        *font = thothterm_font::locator::gdi::parse_log_font(&lf, hdc).ok();
    }
                    ...
                    .resolve_dead_key(leader, (modifiers, vk))
                {
                    ...
                    ResolvedDeadKey::InvalidCombination(c) => {
                            ...
                            inner.keyboard_info.is_dead_key_leader(modifiers, vk)
                        {
                            ...
                            // it done.
                            // <https://github.com/thothterm/thothterm/issues/1729>
                            inner.events.dispatch(WindowEvent::KeyEvent(key.clone()));
                            return Some(0);
                        }
