...
impl CommandDef {
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let thothterm_perform_key_assignment_sel = sel!(thothtermPerformKeyAssignment:);
        ...
        let main_menu = match Menu::get_main_menu() {
            Some(existing) => {
                mark_candidates(
                    ...
                    thothterm_perform_key_assignment_sel,
                );
        ...
        let mut order: Vec<&'static str> = vec!["ThothTerm", "Shell", "Edit", "View", "Window"];
        ...
        for &title in &order {
            for cmd in &commands {
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&cmd.menubar[0], |menu| {
                    if cmd.menubar[0] == "Window" {
                    ...
                    } else if cmd.menubar[0] == "ThothTerm" {
                        menu.assign_as_app_menu();

                        let about_item = MenuItem::new_with(
                            &format!("ThothTerm {}", config::thothterm_version()),
                            Some(thothterm_perform_key_assignment_sel),
                            "",
                        );
                        ...
                        about_item.set_represented_item(RepresentedItem::KeyAssignment(
                            KeyAssignment::CopyTextTo {
                                text: config::thothterm_version().to_string(),
...
pub fn derive_command_from_key_assignment(action: &KeyAssignment) -> Option<CommandDef> {
    Some(match action {
        ...
        ShowSettings => CommandDef {
            brief: "Open Settings".into(),
            doc: "Opens the ThothTerm settings panel".into(),
            keys: vec![(Modifiers::CTRL, ",".into())],
            args: &[ArgType::ActiveWindow],
            menubar: &["Edit"],
            icon: Some("cod_settings_gear"),
        },
        ShowPluginManager => CommandDef {
            brief: "Plugin Manager".into(),
            doc: "Opens the ThothTerm plugin manager".into(),
            keys: vec![(Modifiers::CTRL.union(Modifiers::SHIFT), "p".into())],
            args: &[ArgType::ActiveWindow],
            menubar: &["Edit"],
            icon: Some("cod_extensions"),
        },
        AskAI => CommandDef {
            brief: "Ask AI".into(),
            doc: "Opens the AI assistant panel".into(),
            keys: vec![(Modifiers::CTRL.union(Modifiers::SHIFT), "a".into())],
            args: &[ArgType::ActiveWindow],
            menubar: &["Tools"],
            icon: Some("cod_sparkle"),
        },
        ShowWeb3Status => CommandDef {
            brief: "Web3 Status".into(),
            doc: "Shows Web3 / Ethereum status (chain, gas, balance)".into(),
            keys: vec![(Modifiers::CTRL.union(Modifiers::SHIFT), "w".into())],
            args: &[ArgType::ActiveWindow],
            menubar: &["Tools"],
            icon: Some("cod_globe"),
        },
        ...
        QuitApplication => CommandDef {
            brief: "Quit ThothTerm".into(),
            doc: "Quits ThothTerm".into(),
        ...
        OpenUri(uri) => match uri.as_ref() {
            "https://thothterm.org/" => CommandDef {
                brief: "Documentation".into(),
                doc: "Visit the thothterm documentation website".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("md_help"),
            },
            "https://github.com/thothterm/thothterm/discussions/" => CommandDef {
                brief: "Discuss on GitHub".into(),
                doc: "Visit thothterm's GitHub discussion".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("oct_comment_discussion"),
            },
            "https://github.com/thothterm/thothterm/issues/" => CommandDef {
                brief: "Search or report issue on GitHub".into(),
                doc: "Visit thothterm's GitHub issues".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("fa_ticket"),
...
fn compute_default_actions() -> Vec<KeyAssignment> {
    ...
    return vec![
        ...
        ShowTabNavigator,
        // ----------------- Help
        OpenUri("https://thothterm.org/".to_string()),
        OpenUri("https://github.com/thothterm/thothterm/discussions/".to_string()),
        OpenUri("https://github.com/thothterm/thothterm/issues/".to_string()),
        ShowDebugOverlay,
        ShowSettings,
        ShowPluginManager,
        AskAI,
