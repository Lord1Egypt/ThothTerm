...
use thothterm_bidi::Direction;
use thothterm_client::domain::ClientDomain;
use thothterm_font::shaper::PresentationWidth;
use thothterm_font::FontConfiguration;
use thothterm_gui_subcommands::*;
use thothterm_mux_server_impl::update_mux_domains;
use thothterm_toast_notification::*;

mod colorease;
mod commands;
mod thothterm_startup;
mod customglyph;
mod download;
mod frontend;
mod glyphcache;
...
#[derive(Debug, Parser)]
#[command(
    about = "ThothTerm — GPU terminal for the next 10 years\nhttps://github.com/Lord1Egypt/ThothTerm",
    version = config::thothterm_version()
)]
struct Opt {
    /// Skip loading thothterm.lua
    #[arg(long, short = 'n')]
    skip_config: bool,
...
enum SubCommand {
    ...
    #[command(name = "connect", about = "Connect to thothterm multiplexer")]
...
async fn async_run_ssh(opts: SshCommand) -> anyhow::Result<()> {
    ...
    if opts.verbose {
        ssh_option.insert("thothterm_ssh_verbose".to_string(), "true".to_string());
    }
...
async fn async_run_terminal_gui(
    ...
    std::env::set_var("THOTHTERM_UNIX_SOCKET", unix_socket_path.clone());
    thothterm_blob_leases::register_storage(Arc::new(
        thothterm_blob_leases::simple_tempdir::SimpleTempDir::new_in(&*config::CACHE_DIR)?,
    ))?;
...
impl Publish {
    pub fn resolve(mux: &Arc<Mux>, config: &ConfigHandle, always_new_process: bool) -> Self {
        ...
        match thothterm_client::discovery::resolve_gui_sock_path(
    ...
    pub fn try_spawn(
        ...
        if let Publish::TryPathOrPublish(gui_sock) = &self {
            ...
            match thothterm_client::client::Client::new_unix_domain(None, &dom, false, &mut ui, true)
            {
                Ok(client) => {
                    ...
                    let res = block_on(executor.run(async move {
                        ...
                        if vers.config_file_path
                            != std::env::var_os("THOTHTERM_CONFIG_FILE").map(Into::into)
                        {
                            *self = Publish::NoConnectNoPublish;
                            anyhow::bail!(
                    ...
                    match res {
                        Ok(res) => {
                            log::info!(
                             ...
                             Use thothterm start --always-new-process if you do not want this behavior. \
                             Result={:?}",
                                res
                            );
                            Ok(true)
...
fn spawn_mux_server(unix_socket_path: PathBuf, should_publish: bool) -> anyhow::Result<()> {
    let mut listener =
        thothterm_mux_server_impl::local::LocalListener::with_domain(&config::UnixDomain {
    ...
    std::thread::spawn(move || {
        let name_holder;
        if should_publish {
            name_holder = thothterm_client::discovery::publish_gui_sock_path(
                &unix_socket_path,
                &crate::termwindow::get_window_class(),
            );
...
fn run_terminal_gui(opts: StartCommand, default_domain_name: Option<String>) -> anyhow::Result<()> {
    ...
    // First, let's see if we can ask an already running thothterm to do this.
...
fn notify_on_panic() {
    ...
    std::panic::set_hook(Box::new(move |info| {
        if let Some(s) = info.payload().downcast_ref::<&str>() {
            fatal_toast_notification("ThothTerm panic", s);
        }
        default_hook(info);
    }));
}
...
fn terminate_with_error_message(err: &str) -> ! {
    ...
    fatal_toast_notification("ThothTerm Error", &err);
    std::process::exit(1);
}
...
pub fn run_ls_fonts(config: config::ConfigHandle, cmd: &LsFontsCommand) -> anyhow::Result<()> {
    use thothterm_font::parser::ParsedFont;
    ...
    let font_config = Rc::new(thothterm_font::FontConfiguration::new(
        Some(config.clone()),
    ...
    if let Some(text) = &text {
        ...
        let ft_lib = thothterm_font::ftwrap::Library::new()?;
            ...
            while let Some(info) = iter.next() {
                ...
                if config.custom_block_glyphs {
                    if let Some(block) = info.only_char.and_then(BlockKey::from_char) {
                        ...
                        println!(
                            "{:2} {:4} {:12} drawn by thothterm because custom_block_glyphs=true: {:?}",
...
fn run() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    {
        unsafe {
            ::windows::Win32::UI::Shell::SetCurrentProcessExplicitAppUserModelID(
                ::windows::core::PCWSTR(wide_string("io.github.lordegypt.thothterm").as_ptr()),
            )
            .unwrap();
        }
    }
    ...
    let sub = match opts.cmd.as_ref().cloned() {
        ...
        None => {
            ...
            let mut argv = vec!["thothterm-gui".to_string()];
    ...
    match sub {
        SubCommand::Start(start) => {
            ...
            thothterm_blob_leases::clear_storage();
            res
        }
