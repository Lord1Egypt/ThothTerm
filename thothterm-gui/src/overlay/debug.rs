...
pub fn show_debug_overlay(
    ...
    lua.load("thothterm = require 'thothterm'").exec()?;
    ...
    let version = config::thothterm_version();
    let triple = config::thothterm_target_triple();

    term.render(&[Change::Text(format!(
        "Debug Overlay\r\n\
         thothterm version: {version} {triple}\r\n\
