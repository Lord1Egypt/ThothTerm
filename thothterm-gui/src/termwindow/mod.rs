...
use ::thothterm_term::input::{ClickPosition, MouseButton as TMB};
use ::window::*;
...
use thothterm_dynamic::Value;
use thothterm_font::FontConfiguration;
use thothterm_term::color::ColorPalette;
use thothterm_term::input::LastMouseClick;
use thothterm_term::{Alert, Progress, StableRowIndex, TerminalConfiguration, TerminalSize};
...
lazy_static::lazy_static! {
    static ref WINDOW_CLASS: Mutex<String> = Mutex::new(thothterm_gui_subcommands::DEFAULT_WINDOW_CLASS.to_owned());
...
pub enum TermWindowNotif {
    ...
    GetConfigOverrides(Sender<thothterm_dynamic::Value>),
    SetConfigOverrides(thothterm_dynamic::Value),
...
pub struct TermWindow {
    ...
    pub config_overrides: thothterm_dynamic::Value,
...
impl TermWindow {
    ...
    fn created(&mut self, ctx: RenderContext) -> anyhow::Result<()> {
        ...
        match RenderState::new(ctx, &self.fonts, &self.render_metrics, ATLAS_SIZE) {
            Ok(render_state) => {
                log::debug!(
                    "OpenGL initialized! {} thothterm version: {}",
                    render_info,
                    config::thothterm_version(),
                );
...
impl TermWindow {
    pub async fn new_window(mux_window_id: MuxWindowId) -> anyhow::Result<()> {
        ...
        let myself = Self {
            ...
            window_background,
            config: config.clone(),
            config_overrides: thothterm_dynamic::Value::default(),
            palette: None,
            focused: None,
            mux_window_id,
    ...
    fn dispatch_window_event(
        ...
        match event {
            WindowEvent::Destroyed => {
                ...
                // <https://github.com/thothterm/thothterm/issues/3522>
            ...
            WindowEvent::AppearanceChanged(appearance) => {
                ...
                // <https://github.com/thothterm/thothterm/issues/2295>
                config::reload();
        ...
        match notif {
            ...
            TermWindowNotif::PerformAssignment {
                ...
                let result = || -> anyhow::Result<()> {
                    ...
                    // <https://github.com/thothterm/thothterm/issues/3209>
            ...
            TermWindowNotif::MuxNotification(n) => match n {
                ...
                MuxNotification::Alert {
                    alert: Alert::CommandFailed { exit_code },
                    pane_id,
                } => {
                    if self.window_contains_pane(pane_id) {
                        self.show_ai_error_popup(exit_code);
                    }
                }
                ...
                MuxNotification::TabResized(_) => {
                    // Also handled by thothterm-client
                    self.update_title_post_status();
                }
    ...
    fn emit_status_event(&mut self) {
        ...
        if crate::overlay::ai::is_ai_inferring() {
            prefix.push_str(" 𓆣AI ");
        }

        if let Some(gas) = crate::overlay::web3::cached_gas_status() {
            prefix.push_str(&format!(" ⛽{} ", gas));
        }

        if !prefix.is_empty() {
            self.right_status = format!("{}{}", prefix, self.right_status
                .trim_start_matches(" 𓆣AI ")
...
impl TermWindow {
    ...
    fn activate_tab(&mut self, tab_idx: isize) -> anyhow::Result<()> {
        ...
        // logic in thothterm/src/main.rs. If you update this, update that!
        let max = window.len();
    ...
    fn show_settings(&mut self) {
        ...
        let (overlay, future) = start_overlay(self, &tab, move |_tab_id, term| {
            crate::overlay::show_settings_overlay(term, config)
        });
        self.assign_overlay(tab.tab_id(), overlay);
        promise::spawn::spawn(future).detach();
    }

    fn show_plugin_manager(&mut self) {
        let mux = Mux::get();
        let tab = match mux.get_active_tab_for_window(self.mux_window_id) {
            Some(tab) => tab,
            None => return,
        ...
        let (overlay, future) = start_overlay(self, &tab, move |_tab_id, term| {
            crate::overlay::show_plugins_overlay(term)
        });
        self.assign_overlay(tab.tab_id(), overlay);
        promise::spawn::spawn(future).detach();
    }

    fn show_ai_panel(&mut self) {
        let mux = Mux::get();
        let tab = match mux.get_active_tab_for_window(self.mux_window_id) {
            Some(tab) => tab,
            None => return,
        ...
        let (overlay, future) = start_overlay(self, &tab, move |_tab_id, term| {
            crate::overlay::show_ai_overlay(term)
        });
        self.assign_overlay(tab.tab_id(), overlay);
        promise::spawn::spawn(future).detach();
    }

    fn show_web3_status(&mut self) {
        let mux = Mux::get();
        let tab = match mux.get_active_tab_for_window(self.mux_window_id) {
            Some(tab) => tab,
            None => return,
        ...
        let (overlay, future) = start_overlay(self, &tab, move |_tab_id, term| {
            crate::overlay::show_web3_overlay(term)
        });
        self.assign_overlay(tab.tab_id(), overlay);
        promise::spawn::spawn(future).detach();
    }

    fn show_ai_error_popup(&mut self, exit_code: i32) {
        let mux = Mux::get();
        let tab = match mux.get_active_tab_for_window(self.mux_window_id) {
            Some(tab) => tab,
            None => return,
        ...
        let (overlay, future) = start_overlay(self, &tab, move |_tab_id, term| {
            crate::overlay::ai::show_ai_error_overlay(term, exit_code)
        });
        self.assign_overlay(tab.tab_id(), overlay);
        promise::spawn::spawn(future).detach();
    }
    ...
    fn get_semantic_prompt_zones(&mut self, pane: &Arc<dyn Pane>) -> &[StableRowIndex] {
        ...
        if cache.seqno != seqno {
            ...
            let mut zones: Vec<StableRowIndex> = zones
                .into_iter()
                .filter_map(|zone| {
                    if zone.semantic_type == thothterm_term::SemanticType::Prompt {
                        Some(zone.start_y)
                    } else {
                        None
                    }
            ...
            // <https://github.com/thothterm/thothterm/issues/1121>
            zones.dedup();
            cache.zones = zones;
            cache.seqno = seqno;
        }
    ...
    pub fn perform_key_assignment(
        ...
        match assignment {
            ...
            ShowPluginManager => self.show_plugin_manager(),
            AskAI => self.show_ai_panel(),
            ShowWeb3Status => self.show_web3_status(),
            ...
            OpenUri(link) => {
                thothterm_open_url::open_url(link);
            }
