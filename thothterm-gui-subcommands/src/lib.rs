...
pub const DEFAULT_WINDOW_CLASS: &str = "org.wezfurlong.thothterm";
...
pub struct StartCommand {
    ...
    /// in your thothterm configuration file.
    #[arg(long = "no-auto-connect")]
    pub no_auto_connect: bool,

    /// If enabled, don't try to ask an existing thothterm GUI instance
    /// to start the command.  Instead, always start the GUI in this
    /// invocation of thothterm so that you can wait for the command
    /// to complete by waiting for this thothterm process to finish.
    ...
    /// The default is "org.wezfurlong.thothterm".
    ...
    /// instance of thothterm, including error, update and ssh
    ...
    /// thothterm will simply attach and will NOT spawn the specified PROG.
    /// If you omit --attach when using --domain, thothterm will attach
    /// AND then spawn PROG.
    #[arg(long, requires = "domain")]
    pub attach: bool,

    /// Instead of executing your shell, run PROG.
    /// For example: `thothterm start -- bash -l` will spawn bash
...
pub struct SshCommand {
    ...
    /// `thothterm ssh` is able to parse some (but not all!) options
    ...
    /// For example:
    ///
    /// `thothterm ssh -oIdentityFile=/secret/id_ed25519 some-host`
    #[arg(
        long = "ssh-option",
        short = 'o',
        name = "name=value",
    ...
    /// Instead of executing your shell, run PROG.
    /// For example: `thothterm ssh user@host -- bash -l` will spawn bash
