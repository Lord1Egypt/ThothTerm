...
impl Header {
    fn new(config: &ConfigHandle, size: PtySize, prog: &[&OsStr]) -> Self {
        ...
        env.insert(
            "THOTHTERM_VERSION".to_string(),
            config::thothterm_version().to_string(),
        );
        env.insert(
            "THOTHTERM_TARGET_TRIPLE".to_string(),
            config::thothterm_target_triple().to_string(),
        );
...
pub struct RecordCommand {
    ...
    /// the default_cwd defined by your thothterm configuration
    #[arg(long)]
