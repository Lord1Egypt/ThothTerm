...
use config::{thothterm_version, ConfigHandle};
use mux::Mux;
...
struct ImgCatCommand {
    ...
    /// The default value matches the limit set by thothterm.
...
fn run() -> anyhow::Result<()> {
        ...
        .unwrap_or_else(|| SubCommand::Start(StartCommand::default()))
    {
        ...
        SubCommand::Plugin { cmd } => run_plugin_command(cmd),
    }
}

fn plugin_dir() -> std::path::PathBuf {
    thothterm_plugins::PluginManager::default_dir()
}

fn run_plugin_command(cmd: PluginCommand) -> anyhow::Result<()> {
    ...
    match cmd {
        ...
        PluginCommand::List => {
            ...
            // Overlay .disabled flag for persistent state
            plugins.iter_mut().for_each(|p| {
                if dir.join(&p.name).join(".disabled").exists() {
                    p.enabled = false;
                }
            });
            if plugins.is_empty() {
                println!("No plugins installed. Use `thothterm plugin install <path>`.");
            } else {
                println!("{:<24} {:<8} {:<8} {}", "Name", "Status", "Version", "Description");
                println!("{}", "-".repeat(72));
                for entry in &plugins {
                    let status = if entry.enabled { "enabled" } else { "disabled" };
                    println!(
                        "{:<24} {:<8} {:<8} {}",
                        entry.name, status, entry.version, entry.description
                    );
                }
                let errs: Vec<_> = results.iter().filter(|r| r.is_err()).collect();
                if !errs.is_empty() {
                    eprintln!("\n{} plugin(s) failed to load:", errs.len());
                    for r in errs {
        ...
        PluginCommand::Enable { name } => {
            ...
            anyhow::ensure!(plugin_path.exists(), "Plugin '{}' is not installed", name);
            let flag = plugin_path.join(".disabled");
            if flag.exists() {
        ...
        PluginCommand::Remove { name } => {
            let mut manager = PluginManager::new(&dir);
            manager.load_all();
            manager.remove(&name).map_err(|e| anyhow!("{}", e))?;
