...
pub fn ssh_connect_with_ui(
    ssh_config: thothterm_ssh::ConfigMap,
    ui: &mut ConnectionUI,
...
pub fn ssh_domain_to_ssh_config(ssh_dom: &SshDomain) -> anyhow::Result<ConfigMap> {
    ...
    ssh_config.insert(
        "thothterm_ssh_backend".to_string(),
        match ssh_dom
            .ssh_backend
    ...
    if let Some("true") = ssh_config.get("thothterm_ssh_verbose").map(|s| s.as_str()) {
...
impl RemoteSshDomain {
    ...
    fn build_command(
        ...
        env.insert("THOTHTERM_REMOTE_PANE".to_string(), pane_id.to_string());
...
fn connect_ssh_session(
    ...
    let renderer = termwiz_funcs::new_thothterm_terminfo_renderer();
...
impl Domain for RemoteSshDomain {
    async fn spawn_pane(
        ...
        let StartNewSessionResult { pty, child, writer } = if let Some(session) = session.take() {
                ...
                .context("request ssh pty")
            {
                ...
                Err(err) => {
                    if err
                        .root_cause()
                        .downcast_ref::<thothterm_ssh::DeadSession>()
                        .is_some()
                    {
        ...
        let terminal = thothterm_term::Terminal::new(
            size,
...
impl std::io::Write for PtyWriter {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        // the next byte.
        // <https://github.com/thothterm/thothterm/issues/771>
