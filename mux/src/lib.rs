...
use thiserror::*;
use thothterm_term::{Clipboard, ClipboardSelection, DownloadHandler, TerminalSize};
#[cfg(windows)]
    ...
    let (mut tx, rx) = match allocate_socketpair() {
        ...
        Err(err) => {
            ...
            localpane::emit_output_for_pane(
                pane_id,
                &format!(
                    "⚠️  thothterm: read_from_pane_pty: \
...
impl thothterm_term::DownloadHandler for MuxDownloader {
