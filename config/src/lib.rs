...
use thothterm_dynamic::{FromDynamic, FromDynamicOptions, ToDynamic, UnknownFieldAction, Value};
use thothterm_term::UnicodeVersion;
...
fn xdg_config_home() -> PathBuf {
    match std::env::var_os("XDG_CONFIG_HOME").map(|s| PathBuf::from(s).join("thothterm")) {
        Some(p) => p,
        None => HOME_DIR.join(".config").join("thothterm"),
    }
}
...
fn config_dirs() -> Vec<PathBuf> {
    ...
    if let Some(d) = std::env::var_os("XDG_CONFIG_DIRS") {
        dirs.extend(std::env::split_paths(&d).map(|s| PathBuf::from(s).join("thothterm")));
    }
...
pub fn overridden_config(overrides: &thothterm_dynamic::Value) -> Result<ConfigHandle, Error> {
...
impl ConfigInner {
    ...
    fn accumulate_watch_paths(lua: &Lua, watch_paths: &mut Vec<PathBuf>) {
        if let Ok(mlua::Value::Table(tbl)) = lua.named_registry_value("thothterm-watch-paths") {
    ...
    fn reload(&mut self) {
        ...
        if let Some(path) = file_name {
            ...
            if let Some(parent) = path.parent() {
                ...
                // home dir changes!
                // <https://github.com/thothterm/thothterm/issues/1895>
                if parent != &*HOME_DIR {
    ...
    fn overridden(&mut self, overrides: &thothterm_dynamic::Value) -> Result<ConfigHandle, Error> {
...
impl Configuration {
    ...
    fn overridden(&self, overrides: &thothterm_dynamic::Value) -> Result<ConfigHandle, Error> {
        let mut inner = self.inner.lock().unwrap();
