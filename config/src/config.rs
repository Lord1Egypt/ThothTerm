...
use thothterm_bidi::ParagraphDirectionHint;
use thothterm_config_derive::ConfigMeta;
use thothterm_dynamic::{FromDynamic, ToDynamic};
use thothterm_input_types::{
...
use thothterm_term::TerminalSize;
...
pub struct Config {
    ...
    /// When true, launching a new thothterm instance will prefer
    ...
    /// For example, to have `thothterm` always run `top` by default,
    /// you'd use this:
    ///
    /// ```toml
    /// default_prog = ["top"]
    ...
    ///
    /// and you can set them in thothterm:
    ///
    /// ```toml
    ...
    /// This is off by default because the shift-space
    /// mapping can be annoying in vim.
    #[dynamic(default)]
...
impl Default for Config {
    fn default() -> Self {
        ...
        Config::from_dynamic(
            &thothterm_dynamic::Value::Object(Default::default()),
            Default::default(),
        )
        .unwrap()
    }
...
impl Config {
    pub fn load() -> LoadedConfig {
        Self::load_with_overrides(&thothterm_dynamic::Value::default())
    }
    ...
    pub fn load_with_overrides(overrides: &thothterm_dynamic::Value) -> LoadedConfig {
        ...
        let mut paths = vec![PathPossibility::optional(HOME_DIR.join(".thothterm.lua"))];
        for dir in CONFIG_DIRS.iter() {
            paths.push(PathPossibility::optional(dir.join("thothterm.lua")))
        }
        ...
        if cfg!(windows) {
            ...
            if let Ok(exe_name) = std::env::current_exe() {
                if let Some(exe_dir) = exe_name.parent() {
                    paths.insert(0, PathPossibility::optional(exe_dir.join("thothterm.lua")));
                }
            }
        }
        if let Some(path) = std::env::var_os("THOTHTERM_CONFIG_FILE") {
            log::trace!("Note: THOTHTERM_CONFIG_FILE is set in the environment");
        ...
        // We didn't find (or were asked to skip) a thothterm.lua file, so
        // update the environment to make it simpler to understand this
        // state.
        std::env::remove_var("THOTHTERM_CONFIG_FILE");
        std::env::remove_var("THOTHTERM_CONFIG_DIR");
    ...
    pub fn try_default() -> anyhow::Result<LoadedConfig> {
        let (config, warnings) =
            thothterm_dynamic::Error::capture_warnings(|| -> anyhow::Result<Config> {
    ...
    fn try_load(
        ...
        overrides: &thothterm_dynamic::Value,
        ...
        let (config, warnings) =
            thothterm_dynamic::Error::capture_warnings(|| -> anyhow::Result<Config> {
                ...
                std::env::set_var("THOTHTERM_CONFIG_FILE", p);
                if let Some(dir) = p.parent() {
                    std::env::set_var("THOTHTERM_CONFIG_DIR", dir);
                }
                Ok(cfg)
            });
        let cfg = config?;
    ...
    pub(crate) fn apply_overrides_obj_to<'l>(
        ...
        match overrides {
            thothterm_dynamic::Value::Object(obj) => {
    ...
    pub fn compute_extra_defaults(&self, config_path: Option<&Path>) -> Self {
        ...
        cfg.font_rules.push(StyleRule {
            italic: Some(true),
            intensity: Some(thothterm_term::Intensity::Half),
        ...
        cfg.font_rules.push(StyleRule {
            italic: Some(false),
            intensity: Some(thothterm_term::Intensity::Bold),
            font: bold,
            ..Default::default()
        });
        ...
        cfg.font_rules.push(StyleRule {
            italic: Some(true),
            intensity: Some(thothterm_term::Intensity::Normal),
            font: italic,
            ..Default::default()
        });
    ...
    pub fn apply_cmd_defaults(
        ...
        // and "$APPDIR" set in the thothterm process. These will be
        // propagated to the child processes. Since some apps (including
        // thothterm) use these variables to detect if they are running in
        ...
        cmd.env("TERM_PROGRAM", "ThothTerm");
        cmd.env("TERM_PROGRAM_VERSION", crate::thothterm_version());
    }
}
...
fn default_swap_backspace_and_delete() -> bool {
    // cfg!(target_os = "macos")
    // See: https://github.com/thothterm/thothterm/issues/88
    false
}
...
pub(crate) fn compute_cache_dir() -> anyhow::Result<PathBuf> {
    if let Some(runtime) = dirs_next::cache_dir() {
        return Ok(runtime.join("thothterm"));
    }

    Ok(crate::HOME_DIR.join(".local/share/thothterm"))
}
...
/// Behavior when the program spawned by thothterm terminates
#[derive(Debug, FromDynamic, ToDynamic, Clone, Copy, PartialEq, Eq, Default)]
...
impl FromDynamic for BoldBrightening {
    fn from_dynamic(
        value: &thothterm_dynamic::Value,
        options: thothterm_dynamic::FromDynamicOptions,
    ) -> Result<Self, thothterm_dynamic::Error> {
        match String::from_dynamic(value, options) {
            Ok(s) => match s.as_str() {
                "No" => Ok(Self::No),
                "BrightAndBold" => Ok(Self::BrightAndBold),
                "BrightOnly" => Ok(Self::BrightOnly),
                s => Err(thothterm_dynamic::Error::Message(format!(
...
pub enum ImePreeditRendering {
    /// IME preedit is rendered by ThothTerm itself
    #[default]
    Builtin,
...
/// <https://github.com/thothterm/thothterm/pull/2435>
/// <https://github.com/thothterm/thothterm/issues/2771>
/// <https://github.com/thothterm/thothterm/issues/2630>
