...
pub struct SshDomain {
    ...
    /// <https://thothterm.org/config/lua/pane/get_metadata.html#since_last_response_ms>
    /// instead.
    #[dynamic(default)]
    pub overlay_lag_indicator: bool,

    /// The path to the thothterm binary on the remote host
    pub remote_thothterm_path: Option<String>,
    /// Override the entire `thothterm cli proxy` invocation that would otherwise
    /// be computed from remote_thothterm_path and other information.
    ...
    /// that the remote host have thothterm installed, and is equivalent
    /// to using `thothterm ssh` to connect.
    #[dynamic(default)]
...
impl SshDomain {
    pub fn default_domains() -> Vec<Self> {
        let mut config = thothterm_ssh::Config::new();
        ...
        for host in config.enumerate_hosts() {
            ...
            mux_ssh.push(Self {
                ...
                remote_address: host.to_string(),
                multiplexing: SshMultiplexing::ThothTerm,
                local_echo_threshold_ms: default_local_echo_threshold_ms(),
                ..SshDomain::default()
            });
        }
