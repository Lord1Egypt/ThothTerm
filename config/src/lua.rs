...
use std::path::Path;
use std::sync::Mutex;
use thothterm_dynamic::{
    FromDynamic, FromDynamicOptions, ToDynamic, UnknownFieldAction, Value as DynValue,
};

pub use mlua;

static LUA_REGISTRY_USER_CALLBACK_COUNT: &str = "thothterm-user-callback-count";
...
pub fn get_or_create_sub_module<'lua>(
    ...
    let thothterm_mod = get_or_create_module(lua, "thothterm")?;
    let sub = thothterm_mod.get(name)?;
    match sub {
        Value::Nil => {
            let sub = lua.create_table()?;
            thothterm_mod.set(name, sub.clone())?;
            Ok(sub)
        }
        Value::Table(sub) => Ok(sub),
        wat => anyhow::bail!(
            "cannot register module thothterm.{name} as it is already set to a value of type {}",
            wat.type_name()
        ),
    }
}
        ...
        DynValue::Object(obj) => {
            match obj.get_by_str(&key) {
                None => {
                    ...
                    thothterm_dynamic::Error::warn(message);
                }
                Some(_dvalue) => {
...
/// thothterm specific config paths for lua modules, should
/// they choose to `require` additional code from their config.
///
/// A `thothterm` module is registered so that the script can
/// `require "thothterm"` and call into functions provided by
/// thothterm.  The thothterm module contains:
/// * `executable_dir` - the directory containing the thothterm
///   executable.  This is potentially useful for portable
///   installs on Windows.
/// * `config_dir` - the directory containing the thothterm
///   configuration.
/// * `log_error` - a function that logs to stderr (or the server
///   log file for daemonized thothterm).
/// * `target_triple` - the rust compilation target triple.
/// * `version` - the version of the running thothterm instance.
...
pub fn make_lua_context(config_file: &Path) -> anyhow::Result<Lua> {
    ...
    {
        let globals = lua.globals();
        // This table will be the `thothterm` module in the script
        let thothterm_mod = get_or_create_module(&lua, "thothterm")?;
        ...
        prefix_path(&mut path_array, &crate::HOME_DIR.join(".thothterm"));
        ...
        if let Ok(exe) = std::env::current_exe() {
            if let Some(path) = exe.parent() {
                thothterm_mod
                    ...
                    .context("set thothterm.executable_dir")?;
                if cfg!(windows) {
                    // For a portable windows install, force in this path ahead
                    // of the rest
                    prefix_path(&mut path_array, &path.join("thothterm_modules"));
                }
            }
        }
...
package.searchers[2] = function(module)
  ...
  if name then
    package.loaded.thothterm.add_to_config_reload_watch_list(name)
  end
  return orig(module)
end
        "#,
        ...
        thothterm_mod
            ...
            .context("set thothterm.config_file")?;
        thothterm_mod
            .set(
                "config_dir",
                config_dir
            ...
            .context("set thothterm.config_dir")?;

        lua.set_named_registry_value("thothterm-watch-paths", Vec::<String>::new())?;
        thothterm_mod.set(
            "add_to_config_reload_watch_list",
            lua.create_function(add_to_config_reload_watch_list)?,
        )?;

        thothterm_mod.set("target_triple", crate::thothterm_target_triple())?;
        thothterm_mod.set("version", crate::thothterm_version())?;
        thothterm_mod.set("home_dir", crate::HOME_DIR.to_str())?;
        thothterm_mod.set(
            "running_under_wsl",
        ...
        thothterm_mod.set("font", lua.create_function(font)?)?;
        thothterm_mod.set(
            "font_with_fallback",
            lua.create_function(font_with_fallback)?,
        )?;
        thothterm_mod.set("hostname", lua.create_function(hostname)?)?;
        thothterm_mod.set("action", luahelper::enumctor::Enum::<KeyAssignment>::new())?;
        thothterm_mod.set(
            "has_action",
        ...
        thothterm_mod.set("action_callback", lua.create_function(action_callback)?)?;
        thothterm_mod.set("exec_domain", lua.create_function(exec_domain)?)?;

        thothterm_mod.set("utf16_to_utf8", lua.create_function(utf16_to_utf8)?)?;
        thothterm_mod.set("split_by_newlines", lua.create_function(split_by_newlines)?)?;
        thothterm_mod.set("on", lua.create_function(register_event)?)?;
        thothterm_mod.set("emit", lua.create_async_function(emit_event)?)?;
        thothterm_mod.set("shell_join_args", lua.create_function(shell_join_args)?)?;
        thothterm_mod.set("shell_quote_arg", lua.create_function(shell_quote_arg)?)?;
        thothterm_mod.set("shell_split", lua.create_function(shell_split)?)?;
        ...
        // those values since thothterm was started
...
/// <https://github.com/thothterm/thothterm/issues/1736>
/// <https://github.com/thothterm/thothterm/issues/1786>
...
///
/// `thothterm.font("foo", {foreground="tomato"})`
/// yields:
...
///
/// `thothterm.font_with_fallback({"Operator Mono", "DengXian"})`
///
/// The second optional argument is a list of other TextStyle fields,
/// as described by the `thothterm.font` documentation.
...
/// This implements `thothterm.on`, whose goal is to register an event handler
/// callback.
...
///
/// ```lua
/// thothterm.on("event-name", function(arg1, arg2)
///   -- do something
///   return false -- if you want to prevent other handlers running
/// end);
///
/// thothterm.emit("event-name", "foo", "bar");
/// ```
pub fn register_event<'lua>(
    lua: &'lua Lua,
    (name, func): (String, mlua::Function),
) -> mlua::Result<()> {
    let decorated_name = format!("thothterm-event-{}", name);
...
const IS_EVENT: &str = "thothterm-is-event-emission";
...
pub fn add_to_config_reload_watch_list<'lua>(
    ...
    let mut watch_paths: Vec<String> = lua.named_registry_value("thothterm-watch-paths")?;
    watch_paths.extend_from_slice(&args);
    lua.set_named_registry_value("thothterm-watch-paths", watch_paths)?;
    Ok(())
}
...
mod test {
    ...
    fn can_register_and_emit_multiple_events() -> anyhow::Result<()> {
        ...
        smol::block_on(
            lua.load(
                r#"
local thothterm = require 'thothterm';

thothterm.on('foo', function (n)
...
assert(thothterm.emit('foo', 2) == false)

thothterm.on('bar', function (n, str)
...
assert(thothterm.emit('bar', 42, 'woot') == true)
"#,
            )
            .exec_async(),
        )?;
