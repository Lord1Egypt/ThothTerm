...
/// by the user via `thothterm.time.call_after`.
...
const SCHEDULED_EVENTS: &str = "thothterm-scheduled-events";
...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    thothterm_mod.set("sleep_ms", lua.create_async_function(sleep_ms)?)?;
    thothterm_mod.set("strftime", lua.create_function(strftime)?)?;
    thothterm_mod.set("strftime_utc", lua.create_function(strftime_utc)?)?;
    Ok(())
}
