...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    thothterm_mod.set("nerdfonts", NerdFonts {})?;
    thothterm_mod.set("format", lua.create_function(format)?)?;
    thothterm_mod.set(
        "column_width",
    ...
    thothterm_mod.set("permute_any_mods", lua.create_function(permute_any_mods)?)?;
    thothterm_mod.set(
...
pub fn format_as_escapes(items: Vec<FormatItem>) -> anyhow::Result<String> {
    ...
    let mut renderer = new_thothterm_terminfo_renderer();
...
fn permute_mods<'lua>(
    ...
    use thothterm_input_types::Modifiers;
    ...
    static ref CAPS: Capabilities = {
        ...
        Capabilities::new_with_hints(
            ProbeHints::new_from_env()
                ...
                .colorterm(None)
                .colorterm_bce(None)
                .term_program(Some("ThothTerm".into()))
                .term_program_version(Some(config::thothterm_version().into())),
        )
        .expect("cannot fail to make internal Capabilities")
    };
}

pub fn new_thothterm_terminfo_renderer() -> TerminfoRenderer {
